 ...
 * `net.trilleo.mc.plugins.tritown.commands` package (or any subpackage) to
 * have it automatically discovered and registered at startup.
 *
 * By default every command is registered as a **sub-command** of `/tritown`
 * (alias `/tt`). For example, a command with `name = "reload"` becomes
 * `/tritown reload`. Set [isMainCommand] to `true` to register the command
 ...
 *
 * Example (sub-command – registered as `/tritown start`):
 * ```kotlin
